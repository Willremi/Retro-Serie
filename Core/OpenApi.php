...
class OpenApi
{
    ...
    public function getSerie(string $id): ?array
    {
        ...
        $results = [
            ...
            'dateFirstDiff' => $data['first_air_date'] ? date('d/m/Y', strtotime($data['first_air_date'])) : "Pas d'infos disponibles", 
            // 'dateLastDiff' => date('d/m/Y', strtotime($data['last_air_date'])), 
            'dateLastDiff' => $data['last_air_date'] ? date('d/m/Y', strtotime($data['last_air_date'])) : "Pas d'infos disponibles", 
            'statut' => $data['status'], 
